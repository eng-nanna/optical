...
************************
-->
<section class="product-preview innerPadding">

  <div class="row">
  <?php
    $id = $_GET['id'];
	$qur1="SELECT * FROM glasses WHERE id = '$id'";
	$resulting1= mysqli_query($db_conn, $qur1) or die("Invalid query");
	$rows = mysqli_fetch_array($resulting1);
  ?>
    <div class="small-12 column">
      <nav aria-label="You are here:" role="navigation">
        <ul class="breadcrumbs">
          <li><a href="index.php">Home</a></li>
          <li class="disabled"><?php echo $rows['brand_en']; ?></li>
          <li>
            <span class="show-for-sr">Current: </span> <?php echo $rows['name_en']; ?>
          </li>
        </ul>
      </nav>
    </div><!-- breadcrumbs -->
    ...
    <div class="small-12 medium-7 column">
      <div class="zoomWrap">
      <?php
	    $sql1="SELECT * FROM images WHERE glass_id = '$id'";
		$ans= mysqli_query($db_conn, $sql1) or die("Invalid query");
		$imgs = mysqli_fetch_array($ans);
	  ?>
        <img class="zoomImage" src="img/glasses/<?php echo $imgs['img']; ?>" style="width:510px; height:370px" data-zoom-image="img/glasses/<?php echo $imgs['img']; ?>"/>
      </div>
      <div id="zoomGallery">
      <?php 
	  $sql1="SELECT * FROM images WHERE glass_id = '$id'";
		$ans= mysqli_query($db_conn, $sql1) or die("Invalid query");
		while($imgs = mysqli_fetch_array($ans)){ ?>
        <a href="#" data-image="img/glasses/<?php echo $imgs['img']; ?>"  data-zoom-image="img/glasses/<?php echo $imgs['img']; ?>">
          <img src="img/glasses/<?php echo $imgs['img']; ?>" style="width:70px; height:60px" />
        </a>
        <?php } ?>
      </div>
    </div>
    ...
    <div class="small-12 medium-5 column">
      <div class="product-details">
        <div class="c-name"><?php echo $rows['category_en']; ?></div>
        <h2><?php echo $rows['name_en']; ?></h2>
        <?php
		if ($rows['quantity'] > 0){
		?>
        <div class="secondary label"><?php echo $rows['quantity']; ?> pieces avaliable</div>
        <?php
		}else{
		?>
        <div class="secondary label">Not avaliable now</div>
        <?php
		}
		$q="SELECT * FROM sale WHERE glass_name='$rows[name_en]' AND status_en = 'On Sale'";
		$ans1= mysqli_query($db_conn, $q) or die("Invalid query");
		$count=mysqli_num_rows($ans1);
		if ($count==1){
			$sale_row = mysqli_fetch_array($ans1);
			...
			$new = $old - $percentage;
		?>
        <div class="p-price"><?php echo $new; ?> EGP <small><?php echo $rows['price']; ?> EGP</small></div>
        <?php }else{
			 ?>
             <div class="p-price"><?php echo $rows['price']; ?> EGP</div>
             <?php } ?>
        <div class="p-stars">
           <?php
					  $rate="SELECT AVG(rate) AS average FROM rate WHERE glass_id = '$rows[price]' ";
			  ...
			  ?>
                      ...
                      <?php } ?>
                      <span><?php echo number_format($rate_row['average'], 1, '.', ''); ?></span>
                      <?php } ?>
        </div>



        <form action="" method="post" id="cart-form" accept-charset="utf-8">
<?php
		if ($rows['quantity'] > 0){
		?>
          <div class="p-quantity">
            <p>Quantity</p>
            <a class="minus">-</a>
            <input type="hidden" name="glass_id" value="<?php echo $id; ?>">
            <input type="number" name="quantity" value="1" placeholder="">
            <a class="add">+</a>
          </div><!-- p-quantity -->

          <input data-open="cart-list-reveal" type="button" class="large button expanded radius" name="submit" id="cart" value="Add To Cart">
          <?php } ?>
...
<section class="product-description">
  <div class="row">
    <div class="small-12 column">
     <?php
		$sql3="SELECT * FROM details WHERE glass_id='$rows[id]'";
		$ans3= mysqli_query($db_conn, $sql3) or die("Invalid query");
		$count=mysqli_num_rows($ans1);
		$info = mysqli_fetch_array($ans3);
		?>
      <ul class="tabs" data-tabs id="description-tabs">
        <li class="tabs-title is-active"><a href="#panel1" aria-selected="true">Details</a></li>
        <li class="tabs-title"><a href="#panel2">FRAME/LENSES</a></li>
        <li class="tabs-title"><a href="#panel3">Size</a></li>
      </ul>
      <div class="tabs-content" data-tabs-content="description-tabs">
        <div class="tabs-panel is-active" id="panel1">
          <p><strong>Frame color</strong>: <?php echo $rows['color_en']; ?></p>
          <p><?php echo $rows['description_en']; ?></p>
        </div>
        <div class="tabs-panel" id="panel2">
          <p><strong>Frame Material:</strong> <?php echo $info['frame_material']; ?></p>
          <p><strong>Frame Color:</strong> <?php echo $rows['color_en']; ?></p>
          <p><strong>Lenses:</strong> <?php echo $info['lenses']; ?></p>
        </div>
        <div class="tabs-panel" id="panel3">
          <p><strong>Shape:</strong> <?php echo $info['shape']; ?></p>
          <p><strong>Size Lens-Bridge: </strong> <?php echo $info['bridge']; ?></p>
          <p><strong>Temple Length:</strong> <?php echo $info['temple']; ?></p>
        </div>
      </div>
    </div>
  </div>
...
<div class="reveal" id="cart-list-reveal" data-reveal>
