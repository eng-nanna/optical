...
?>
      ...
      <?php
		  $name = $_GET['brand'];
		  $query="SELECT * FROM brand where brand_en = '$name'";
		  $result= mysqli_query($db_conn, $query) or die("Invalid query");
		  $row = mysqli_fetch_array($result);
		 ...
		 if (isset($_POST['filter'])){
						 ...
						 $sql1="SELECT * FROM glasses where brand_en = '$row[brand_en]' AND $start < price  AND price < $end AND quantity <> 0";
						 
						 if (isset($_POST['gender'])){
						$gender = implode(',', $_POST['gender']);
					    $sql1="SELECT * FROM glasses where brand_en = '$row[brand_en]' AND $start < price  AND price < $end AND category_en IN ('$gender') AND quantity <> 0";
						}
						
						if (isset($_POST['sale-filter'])){
							 $sql1= "SELECT
							 glasses.name_en, glasses.color_en, glasses.brand_en, glasses.price, glasses.id,
							 sale.glass_name, sale.discount, sale.status_en
							 FROM sale
							 INNER JOIN glasses
								 ON sale.glass_name = glasses.name_en AND glasses.brand_en = '$name' AND sale.status_en = 'On Sale' AND glasses.quantity <> 0";
							  }
					  }else{
					$sql1="SELECT * FROM glasses where brand_en = '$row[brand_en]' AND quantity <> 0";
					}
        ...
        <h1 class="brand-name">
          <figure>
            <img src="img/brands/<?php echo $row['pic']; ?>" width="200" alt="<?php echo $row['brand_en']; ?>">
          </figure>
          <?php echo $row['brand_en']; ?>
        ...
        <div class="row">
          ...
          <div class="medium-12 column">
            <nav aria-label="You are here:" role="navigation">
              <ul class="breadcrumbs">
                ...
                <li>
                  <span class="show-for-sr">Current: </span> <?php echo $row['brand_en']; ?>
                </li>
              </ul>
            </nav>
          </div><!-- breadcrumbs -->
          ...
          <div class="small-12 medium-2 column homeBrandList">
            <aside>
              <div class="brand-cat-logo show-for-medium">
                <figure>
                  <img src="img/brands/<?php echo $row['pic']; ?>" width="200" alt="<?php echo $row['brand_en']; ?>" />
                </figure>
              </div><!-- brand-name -->
              ...
              <section id="categoryFilters">
                <form class="category-filter" name="cat-filter" action="<?php $_SERVER['PHP_SELF']?>" method="post">
                  ...
                  <ul class="menu vertical filter-gender">
                    <li>
                      <label>
                        <input type="checkbox" name="gender[]" value="men glasses">
                        <i class="icon-mustache"></i> Men
                      </label>
                    </li>
                    <li>
                      <label>
                        <input type="checkbox" name="gender[]" value="women glasses">
                        <i class="icon-lips"></i> Women
                      </label>
                    </li>
                    <li>
                      <label>
                        <input type="checkbox" name="gender[]" value="Children Glasses">
                        <i class="icon-toy"></i> Kids
                      </label>
                    </li>
                    <li>
                      <label>
              ...
              <nav class="show-for-medium">
                <ul class="menu vertical">
                  ...
                  <?php
					  $sql="SELECT * FROM brand WHERE brand_en <> '$row[brand_en]'";
					  $res= mysqli_query($db_conn, $sql) or die("Invalid query");
					  while($rows = mysqli_fetch_array($res)){
					  ?>
                  <li><a href="brand.php?brand=<?php echo $rows['brand_en']; ?>" title="<?php echo $rows['brand_en']; ?>"><?php echo $rows['brand_en']; ?></a></li>
                   <?php }?>
                </ul>
              </nav>
            </aside>
            ...
            <div class="row">
				    ...
				    $start_from = ($page-1) * $num_rec_per_page;
					 if (isset($_POST['filter'])){
						 $start = intval($_POST['start']);
						 $end = intval($_POST['end']);
						 $sql1="SELECT * FROM glasses where brand_en = '$row[brand_en]' AND $start < price  AND price < $end AND quantity <> 0 LIMIT $start_from, $num_rec_per_page";
						 
						 if (isset($_POST['gender'])){
						$gender = implode(',', $_POST['gender']);
					    $sql1="SELECT * FROM glasses where brand_en = '$row[brand_en]' AND $start < price  AND price < $end AND category_en IN ('$gender') AND quantity <> 0 LIMIT $start_from, $num_rec_per_page";
						}
						 ...
						 if (isset($_POST['sale-filter'])){
							 ...
							 FROM sale
							 INNER JOIN glasses
								 ON sale.glass_name = glasses.name_en AND glasses.brand_en = '$name' AND sale.status_en = 'On Sale' AND glasses.quantity <> 0 LIMIT $start_from, $num_rec_per_page";
							  }
					  }else{

					  $sql1="SELECT * FROM glasses where brand_en = '$row[brand_en]' AND quantity <> 0 LIMIT $start_from, $num_rec_per_page";
					  }
					  $res1= mysqli_query($db_conn, $sql1) or die("Invalid query");
    ...
    echo "<li class='pagination-previous'><a href='glasses.php?page=1'>".'First'."</a><span class='show-for-sr'>page</span></li> "; // Goto 1st page  
